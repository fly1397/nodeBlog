 ...
 * @param {Boolean} in_post  是否在其他板块？ forgive 为 false
 * @param {Function} callback 回调函数
 */
exports.sendMessageToMentionUsers = function (text, topicId, authorId,in_post, reply_id, callback) {
  ...
  User.getUsersByNames(fetchUsers(text), function (err, users) {
    ...
    users.forEach(function (user) {
      Message.sendAtMessage(user._id, authorId, topicId, reply_id,in_post, ep.done('sent'));
    });
  });
};
...
exports.linkUsers = function (text, callback) {
  ...
  for (var i = 0, l = users.length; i < l; i++) {
    ...
    text = text.replace(new RegExp('@' + name + '\\b', 'g'), '<a href="/user/' + name + '">@' + name + '</a>');
  }
  if (!callback) {
    return text;
  }
