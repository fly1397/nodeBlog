        ...
        topics_id.forEach(function (id, i) {
            exports.getTopicById(id, proxy.group('topic_ready', function (topic, author, last_reply) {
                ...
                if (topic) {
                   ...
                   var summary =topic.content.replace(/<[^>]+>/g,"").substr(0, 80)+"...";
                    var tn = topic.content.match(/<img.*src=[\"](.*?)[\"].*?>/gi);
                    if(tn){
                        tn = tn[0]
                    }else{
                        tn=""
...
exports.getFullTopicT = function (title, callback) {
    var proxy = new EventProxy();
    var events = ['topic', 'author', 'replies'];
    proxy
        .assign(events, function (topic, author, replies) {
    ...
    Topic.findOne({title: title}, proxy.done(function (topic) {
        if (!topic) {
            proxy.unbind();
            return callback(null, '此话题不存在或已被删除。');
        }
