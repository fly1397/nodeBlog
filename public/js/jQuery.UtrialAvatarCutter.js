jQuery.UtrialAvatarCutter = function(config){
    ...
    this.outs= function(ele,ele2){
        setTimeout('$(ele).removeAttr("style")',20);
        $(ele2).remove();
    }
	...
	this.init = function(){
		...
		url = $('#'+img_id).attr("src").replace(/\?date=.*/,"");
		$("<img/>") // 在内存中创建一个img标记
		.attr("src", url+"?date="+Math.random(1))
		.load(function() {
			picRealWidth = this.width;
			picRealHeight = this.height;
			var W=picRealWidth || 100,H=picRealHeight ||100;
			var warp_w = $('#'+img_id).width();
			var warp_h = $('#'+img_id).height();
			if(W<warp_w){
				ratio = 1;
			}else{
				ratio = W/warp_w > H/warp_h ?W/warp_w:H/warp_h;
			}
		});
	...
	this.submit = function(){
        return {w:w,h:h,x:x,y:y,r:ratio,url:url};
    }
}