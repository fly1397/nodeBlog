...
var respond = function(static_url, callback) {
    return function(req, res, next) {
        ...
        if (req.query.action === 'config') {
            ...
            var mkFN  = tools.mkdirsSync('public/uploads/'+userId+'/files');
            if(!mkFN){
                return res.send('{state : "出错啦，请重新上传试试！"}');
            }
            var form = new formidable.IncomingForm();
            form.uploadDir='public/uploads/'+userId+'/files';
            form.parse(req, function(error, fields, files){
                res.ue_up = function(img_url) {
                    var tmpdir = path.join(os.tmpDir(), path.basename(files.upfile.name));
                    var name = snowflake.nextId() + path.extname(tmpdir);
                    var dest = path.join(static_url, img_url, name);
                    fs.renameSync(files.upfile.path, "public/uploads/"+userId+"/files/"+name);
                    res.json({
