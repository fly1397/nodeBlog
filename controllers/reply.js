...
exports.add = function (req, res, next) {
  ...
  var switches;
  Post.getPost(topic_id,ep.doneLater(function(topic){
    if(topic){
       switches = Post;
    }else{
      switches = Topic;
  ...
  ep.all('topic', 'topic_author', function (topic, topicAuthor) {
    Reply.newAndSave(content, topic_id, req.session.user._id, reply_id,in_post, ep.done(function (reply) {
      Topic.updateLastReply(topic_id, reply._id, ep.done(function () {
        ep.emit('reply_saved', reply);
        //发送at消息，并防止重复 at 作者
        var newContent = content.replace('@' + topicAuthor.loginname + ' ', '');
        at.sendMessageToMentionUsers(newContent, topic_id, req.session.user._id,in_post, reply._id);
      }));
    }));
  ...
  ep.all('reply_saved', 'topic', function (reply, topic) {
    if (topic.author_id.toString() !== req.session.user._id.toString()) {
      message.sendReplyMessage(topic.author_id, req.session.user._id, topic._id, reply._id,in_post);
    }
    ep.emit('message_saved');
  });

  ep.all('reply_saved', 'message_saved', 'score_saved', function (reply) {
    res.redirect('/forgive/' + topic_id + '#' + reply._id);
  });
};
...
exports.add2 = function (req, res, next) {
  ...
  ep.all('topic', 'topic_author', function (topic, topicAuthor) {
    Reply.newAndSave(content, topic_id, req.session.user._id, reply_id,in_post, ep.done(function (reply) {
      Post.updateLastReply(topic_id, reply._id, ep.done(function () {
        ep.emit('reply_saved', reply);
        //发送at消息，并防止重复 at 作者
        var newContent = content.replace('@' + topicAuthor.loginname + ' ', '');
        at.sendMessageToMentionUsers(newContent, topic_id, req.session.user._id,in_post, reply._id);
  ...
  ep.all('reply_saved', 'message_saved', 'score_saved', function (reply) {
    res.redirect('/post/' + topic_id + '#' + reply._id);
  });
};
