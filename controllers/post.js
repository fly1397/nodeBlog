...
exports.index = function (req, res, next) {
    ...
    ep.fail(next);
    Post.getFullPost(topic_id, ep.done(function (message, topic, author, replies) {
        if (message) {
            ep.unbind();
        ...
        if (!req.session.user) {
            ...
            PostCollect.getTopicCollect(req.session.user._id, topic._id, ep.done(function (doc) {
        ...
        Post.getPostsByQuery(query, options, ep.done('other_topics'));

        // get no_reply_topics
        var options2 = { limit: 5, sort: '-create_at'};
        Post.getPostsByQuery({reply_count: 0}, options2, ep.done('no_reply_topics'));
    }));
};
...
exports.put = function (req, res, next) {
    ...
    var allTabs = config.post_tabs.map(function (tPair) {
        return tPair[0];
    });
    ...
    Post.newAndSave(title, content, req.session.user._id, tab, function (err, topic) {
        if (err) {
            return next(err);
        }
        ...
        //发送at消息
        var in_post=true;
        at.sendMessageToMentionUsers(content, topic._id, req.session.user._id,in_post);
    });
};
...
exports.showEdit = function (req, res, next) {
    ...
    Post.getPostById(topic_id, function (err, topic, tags) {
        if (!topic) {
...
exports.delete = function (req, res, next) {
    ...
    Post.getPost(topic_id, function (err, topic) {
        if (err) {
...
exports.good = function (req, res, next) {
    ...
    var referer = req.get('referer');
    Post.getPost(topicId, function (err, topic) {
        if (err) {
            return next(err);
        }
        if (!topic) {
...
exports.collect = function (req, res, next) {
    ...
    Post.getPost(topic_id, function (err, topic) {
        ...
        PostCollect.getTopicCollect(req.session.user._id, topic._id, function (err, doc) {
            if (err) {
                return next(err);
            }
            if (doc) {
            ...
            PostCollect.newAndSave(req.session.user._id, topic._id, function (err) {
                if (err) {
                    return next(err);
                }
                res.json({status: 'success'});
...
exports.de_collect = function (req, res, next) {
    ...
    Post.getPost(topic_id, function (err, topic) {
        ...
        PostCollect.remove(req.session.user._id, topic._id, function (err) {
            if (err) {
                return next(err);
            }
            res.json({status: 'success'});
