...
exports.index = function (req, res, next) {
    ...
    if (topic_id.length !== 24) {
        return res.render('notify/notify', {
            error: '此话题不存在或已被删除。', referer: referer
        });
    }
    ...
    Topic.getFullTopic(topic_id, ep.done(function (message, topic, author, replies) {
        if (message) {
            ep.unbind();
            return res.render('notify/notify', { error: message , referer: referer});
        }
...
exports.index2 = function (req, res, next) {
    ...
    var topic_title = req.params.title;
    ...
    ep.fail(next);
    Topic.getFullTopicT(topic_title, ep.done(function (message, topic, author, replies) {
        if (message) {
            ep.unbind();
            return res.render('notify/notify', { error: message , referer: referer});
        }
...
exports.put = function (req, res, next) {
    ...
    Topic.newAndSave(title, content, tab, req.session.user._id, function (err, topic) {
        ...
        proxy.all('score_saved', function () {
            res.redirect('/forgive/' + topic._id);
        });
        proxy.fail(next);
        ...
        if(Target){
            // 发送邮件
            mail.sendPostMail(Target, req.session.user.email, tab,topic);
        }
        //发送at消息
...
exports.showEdit = function (req, res, next) {
    ...
    Topic.getTopicById(topic_id, function (err, topic, tags) {
        if (!topic) {
            res.render('notify/notify', {error: '此话题不存在或已被删除。', referer: referer});
            return;
        }
        ...
        if (String(topic.author_id) === String(req.session.user._id) || req.session.user.is_admin) {
            ...
            res.render('notify/notify', {error: '对不起，你不能编辑此话题。', referer: referer});
        }
    });
};
