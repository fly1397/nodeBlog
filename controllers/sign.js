...
exports.showSignup = function (req, res) {
    ...
    res.render('sign/signup',{inviter:inviter});
};
exports.signup = function (req, res, next) {
    ...
    var emailTA = validator.trim(req.body.emailTA);
    var ep = new eventproxy();
    ep.fail(next);
    ep.on('prop_err', function (msg) {
        res.status(422);
        res.render('sign/signup', {error: msg, loginname: loginname, email: email,emailTA:emailTA});
    });
    ...
    User.getUsersByQuery({'$or': [
        ...
        var avatarUrl = "";
        User.newAndSave(loginname, loginname, password, email, avatarUrl, config.active, function (err) {
            if (err) {
                return next(err);
            }
            // 发送邀请邮件
            if(emailTA){
                mail.sendInviteMail(emailTA, email,loginname);
            }
            if(!config.active){
                // 发送激活邮件
                mail.sendActiveMail(email, utility.md5(email + password + config.session_secret), loginname);
                res.render('sign/signup', {
                    success: '欢迎加入 ' + config.name + '！我们已给您的注册邮箱发送了一封邮件，请点击里面的链接来激活您的帐号。'
                });
            }else{
                res.render('sign/signin', {
                    success: '注册成功，欢迎加入 ' + config.name + '。赶快登录吧！',
                    loginname: loginname
                });
            }
...
exports.update_pass = function (req, res, next) {
    ...
    User.getUserByNameAndKey(name, key, ep.done(function (user) {
        ...
        var md5 = crypto.createHash('md5'),
            passhash = md5.update(psw).digest('hex');
